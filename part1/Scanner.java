...
public class Scanner {
	...
	private String commentChar = ":"; // Use for a comment (both start and closing)
  ...
  // Goes past all values provided from the given set
  ...
  // skipping past a comment
	private void nextComment() {
	
		pos++; // Advances character from original ':'
		String c;

		for(int i = pos; i < program.length(); i++){
			c = program.charAt(i)+"";
			pos++;
			if(commentChar.compareTo(c) == 0){ //Ending found!
				break;
			}
		}

		next(); // Continuing the scanning
		
	}

  // finding out what what keyword it is
  ...
  // finding the type of operator that is used
	private void nextOp() {
		int old=pos;
		pos=old+2;
		if (!done()) {
	...
	public boolean next() {
		...
		if (digits.contains(c))
			...
			nextOp();
		else if (commentChar.compareTo(c) == 0){ // Searching if the code is a comment
			nextComment();
		}
		else {
  ...
  // Grabs the current token in the scanner
  ...
  // Grabs the current position where the scanner is
	public int pos() {
		return pos;
	}
